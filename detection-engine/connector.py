...
# Group names containing any of these are treated as "elevated" for the
# IAM_USER_NO_MFA rule's `in_elevated_group` context key. Simple heuristic,
# same spirit as P2's name-based derived signals -- documented here so it's
# easy to extend if the real account uses different naming.
ELEVATED_GROUP_HINTS = ("admin", "poweruser", "power-user", "root", "superuser")
...
class AWSConnector:
    ...
    def get_account_id(self):
        """Cached account id, needed to build correct root/account ARNs."""
        if self._account_id_cache is None:
            try:
                self._account_id_cache = self.sts_client.get_caller_identity()["Account"]
            except ClientError:
                self._account_id_cache = "unknown-account"
    ...
    def get_iam_config(self):
        ...
        try:
            ...
            for user in users.get("Users", []):
                ...
                # Console access: a login profile exists only if the user has a console password set.
                ...
                try:
                    ...
                    # any other error: leave has_console_access as unknown-default True,
                    # rather than guessing -- matches connector's existing "don't crash" pattern

                # Group memberships, used for the elevated-permissions heuristic.
                in_elevated_group = False
                try:
                    groups = self.iam_client.list_groups_for_user(UserName=user_name)
                    group_names_lower = [g["GroupName"].lower() for g in groups.get("Groups", [])]
                    in_elevated_group = any(
                        hint in gname for gname in group_names_lower for hint in ELEVATED_GROUP_HINTS
                    )
                except ClientError:
                    pass
                ...
                iam_data["users"].append({
                    ...
                    "in_elevated_group": in_elevated_group,
                })
        except ClientError as err:
